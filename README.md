# Highball

Highball is an NYC transit planning sandbox for drafting proposed subway corridors, simulating demand impact, and comparing proposed travel time/cost against current service.

It includes:

- FastAPI backend for simulation and ML-backed inference
- React + TypeScript + Vite frontend with an interactive map
- Data ingestion/parsing scripts for subway, bus, and census inputs
- Model training scripts for ridership, operating cost, and route travel time

## 0. 5-Minute Quickstart

If you only want to run the app quickly (without rebuilding all data/models):

```bash
# 1) install dependencies
make setup

# 2) run backend (terminal A)
make backend

# 3) run frontend (terminal B)
make frontend
```

Open:

- frontend: `http://localhost:5173`
- backend health: `http://localhost:8000/health`

Quick sanity checks:

```bash
# backend syntax
source .venv/bin/activate && python -m py_compile src/api.py

# frontend build
cd frontend && npm run build
```

When to skip this and use the full guide:

- if you need fresh pulls from NY Open Data
- if you changed training code or want to retrain models
- if you need to regenerate `data/processed` artifacts

## 1. Tech Stack

- Python backend: FastAPI, NetworkX, XGBoost, pandas/geopandas
- Frontend: React 19, TypeScript, Vite, Leaflet
- Data sources: GTFS, NY Open Data (Socrata), Census API, OSM

## 2. Repository Structure

- [src](src): backend API + path utilities
- [frontend](frontend): web app
- [data](data): data pull/parsing/training scripts and artifacts
- [data/raw](data/raw): raw ingested datasets
- [data/processed](data/processed): cleaned/engineered tables and graphs
- [data/models](data/models): trained model artifacts used at runtime
- [Makefile](Makefile): common setup/run helpers

## 3. Prerequisites

- Python 3.11+
- Node.js 18+
- npm
- make
- curl + unzip

Linux note:

- XGBoost needs OpenMP runtime (typically available via libgomp on Linux).

macOS note:

- install libomp (for XGBoost): `brew install libomp`

## 4. First-Time Setup

From repo root:

```bash
make setup
```

This creates:

- Python virtual environment at `.venv`
- Python dependencies from [requirements.txt](requirements.txt)
- Frontend dependencies from [frontend/package.json](frontend/package.json)
- base directories and `.env` template

Optional: activate the venv manually for direct script runs:

```bash
source .venv/bin/activate
```

## 5. Running the App Locally

Use two terminals.

Terminal 1 (backend):

```bash
make backend
```

Terminal 2 (frontend):

```bash
make frontend
```

Endpoints:

- Backend health: `http://localhost:8000/health`
- Frontend: `http://localhost:5173`

## 6. Environment Variables

A template `.env` is generated by `make setup`.

Common variables:

- `NYC_OPEN_DATA_APP_TOKEN=` optional Socrata token for higher limits
- `SOCRATA_APP_TOKEN=` optional token for pull scripts
- `MTA_BUS_RESOURCE_ID=` optional default bus dataset id
- `CENSUS_API_KEY=` required only for census fetch in [data/fetch_features.py](data/fetch_features.py)

## 7. Data Pull and Processing

### 7.1 Minimal Runtime Data (if you need to rebuild artifacts)

If your repo already contains populated [data/processed](data/processed) and [data/models](data/models), you can skip this section.

### 7.2 Subway GTFS + ridership pulls via Makefile

```bash
make data
```

This downloads into [data/raw](data/raw):

- `gtfs_subway.zip` + extracted `gtfs_subway/`
- `mta_ridership.csv`

### 7.3 Build GTFS time graph used by routing

From repo root:

```bash
source .venv/bin/activate
python src/build_graph.py
```

Output:

- [data/processed/mta_time_graph.json](data/processed/mta_time_graph.json)

### 7.4 Parse subway hourly ridership into model-ready files

```bash
source .venv/bin/activate
python data/mta_parser.py \
  --input data/raw/MTA_Subway_Hourly_Ridership__2020-2024_20260426.csv \
  --output-dir data/processed
```

Primary outputs:

- [data/processed/stations.json](data/processed/stations.json)
- [data/processed/ridership_daily.csv](data/processed/ridership_daily.csv)
- [data/processed/ridership_hourly.csv](data/processed/ridership_hourly.csv)
- [data/processed/line_summary.csv](data/processed/line_summary.csv)
- [data/processed/hourly_patterns.json](data/processed/hourly_patterns.json)
- [data/processed/network_graph.json](data/processed/network_graph.json)

### 7.5 Pull monthly ridership and monthly operating cost labels

```bash
source .venv/bin/activate
python data/ridership_pull.py --app-token "$SOCRATA_APP_TOKEN"
python data/cost_pull.py --app-token "$SOCRATA_APP_TOKEN"
```

Outputs:

- [data/processed/ridership_monthly.csv](data/processed/ridership_monthly.csv)
- [data/raw/monthly_operating_cost.csv](data/raw/monthly_operating_cost.csv)

### 7.6 Optional: fetch census + OSM features and station feature table

```bash
source .venv/bin/activate
python data/fetch_features.py --census-key "$CENSUS_API_KEY"
```

Output used by model training/inference:

- [data/raw/station_features.csv](data/raw/station_features.csv)

### 7.7 Optional: bus data pipeline

Pull:

```bash
source .venv/bin/activate
python data/fetch_bus_data.py --resource-id "$MTA_BUS_RESOURCE_ID" --app-token "$SOCRATA_APP_TOKEN"
```

Parse:

```bash
source .venv/bin/activate
python data/bus_parser.py \
  --input data/raw/MTA_Bus_Hourly_Ridership.csv \
  --output-dir data/processed
```

## 8. Model Training

### 8.1 Ridership + peak-factor models

```bash
source .venv/bin/activate
python data/train_model.py \
  --features data/raw/station_features.csv \
  --hourly-csv data/raw/MTA_Subway_Hourly_Ridership__2020-2024_20260426.csv \
  --out-dir data/models
```

Artifacts:

- [data/models/ridership_model.json](data/models/ridership_model.json)
- [data/models/peak_factor_model.json](data/models/peak_factor_model.json) (if hourly CSV supplied)
- [data/models/feature_columns.json](data/models/feature_columns.json)
- [data/models/training_report.json](data/models/training_report.json)

### 8.2 Monthly operating cost model

```bash
source .venv/bin/activate
python data/price_train.py \
  --labels-csv data/raw/monthly_operating_cost.csv \
  --ridership-monthly-csv data/processed/ridership_monthly.csv \
  --line-summary-csv data/processed/line_summary.csv \
  --out-dir data/models
```

Artifacts:

- [data/models/cost_model.json](data/models/cost_model.json)
- [data/models/cost_feature_columns.json](data/models/cost_feature_columns.json)
- [data/models/cost_training_report.json](data/models/cost_training_report.json)

### 8.3 Time model for proposed-corridor travel time

Create training set from GTFS graph:

```bash
source .venv/bin/activate
python data/timegraph_parser.py
```

Train and save model:

```bash
source .venv/bin/activate
python -c "from data.time_predict import train_model, save_model; m=train_model(); save_model(m)"
```

Artifacts:

- [data/processed/time_training_data.csv](data/processed/time_training_data.csv)
- [data/models/time_model.joblib](data/models/time_model.joblib)

## 9. Testing and Validation

There is no formal pytest suite currently. Use the following checks.

### 9.1 Backend syntax check

```bash
source .venv/bin/activate
python -m py_compile src/api.py data/time_predict.py src/predict.py
```

### 9.2 Frontend build check

```bash
cd frontend
npm run build
```

### 9.3 Frontend lint

```bash
cd frontend
npm run lint
```

### 9.4 API smoke test

Start backend, then run:

```bash
curl -X POST http://localhost:8000/api/simulate \
  -H "Content-Type: application/json" \
  -d '{
    "train_service": "local",
    "stations": [
      {"id":"127","name":"Times Sq-42 St","lat":40.75529,"lon":-73.987495,"is_new":false},
      {"id":"124","name":"34 St-Penn Station","lat":40.750373,"lon":-73.991057,"is_new":false}
    ]
  }'
```

You should receive JSON with fields such as:

- `new_line_ridership`
- `operational_cost_monthly`
- `route_comparison.is_walking_only`

## 10. Dev Workflow

Typical local loop:

```bash
# one-time
make setup

# each session
make backend
make frontend

# when changing backend logic
source .venv/bin/activate && python -m py_compile src/api.py

# when changing frontend
cd frontend && npm run build
```

## 11. Notes About Make Targets

Current [Makefile](Makefile) includes `preprocess` and `train` targets that call `src/preprocess.py` and `src/train.py`, but those files are not present in this repository right now.

For now, use the explicit script commands documented above in sections 7 and 8.

## 12. Troubleshooting

- `make backend` exits with code 130: usually a manual Ctrl+C stop, not a crash.
- `libomp` errors on macOS: run `brew install libomp`.
- Missing model artifacts at runtime: ensure files exist under [data/models](data/models).
- Missing graph or station data: regenerate [data/processed/mta_time_graph.json](data/processed/mta_time_graph.json) and [data/processed/stations.json](data/processed/stations.json).
- Socrata requests fail/rate-limit: set `SOCRATA_APP_TOKEN` or `NYC_OPEN_DATA_APP_TOKEN`.

## 13. License

See [LICENSE](LICENSE).

## 14. Deployment / Production Run

This project does not yet include container/IaC deployment files, so use this section as the baseline production runbook.

### 14.1 Backend (FastAPI)

Use an environment-specific host/port and run behind a reverse proxy.

```bash
source .venv/bin/activate
uvicorn src.api:app --host 0.0.0.0 --port 8000 --workers 2
```

Recommended production setup:

- reverse proxy with TLS (for example Nginx/Caddy)
- process supervisor (`systemd`, `supervisord`, or equivalent)
- separate runtime user (non-root)
- pre-provisioned artifacts under [data/models](data/models) and [data/processed](data/processed)

Minimal backend preflight before deploy:

```bash
source .venv/bin/activate
python -m py_compile src/api.py data/time_predict.py src/predict.py
curl http://127.0.0.1:8000/health
```

### 14.2 Frontend (Vite build)

Build static assets:

```bash
cd frontend
npm ci
npm run build
```

Then serve [frontend/dist](frontend/dist) from static hosting or a reverse proxy.

Important:

- ensure frontend can reach backend API URL (same-origin proxy or CORS-allowed host)
- rebuild frontend when API payload contracts change

### 14.3 Release Checklist (Runtime)

- backend starts cleanly and `/health` returns `{"status":"ok"}`
- frontend build succeeds
- `/api/simulate` smoke request returns 200 with expected keys
- required model/data artifacts are present
- environment variables are set (`SOCRATA_APP_TOKEN`, optional app tokens)

## 15. Contributor Workflow

### 15.1 Branching

- create feature branches from `main`
- use short, purpose-based names (examples: `feature/suggest-line`, `fix/time-scaling`)
- keep PRs scoped to one concern when possible

```bash
git checkout main
git pull
git checkout -b feature/your-change
```

### 15.2 Local Validation Before Push

Run at least:

```bash
# backend compile check
source .venv/bin/activate
python -m py_compile src/api.py data/time_predict.py src/predict.py

# frontend typecheck/build
cd frontend
npm run build
```

If your change touches data/model code, also run the relevant training/data scripts from sections 7 and 8 and confirm artifacts are generated.

### 15.3 Commit + Push

```bash
git add -A
git commit -m "Describe what changed"
git push -u origin feature/your-change
```

Commit guidance:

- subject line in imperative mood
- include the subsystem prefix where useful (api, frontend, data)

### 15.4 Pull Request Expectations

In PR description include:

- what changed
- why it changed
- how it was tested (commands + outcome)
- any data/model artifacts that must be regenerated

For UI changes, include screenshots or short screen recordings.

For API changes, include request/response examples.

### 15.5 Merge + Post-Merge

- rebase/merge `main` into your branch and resolve conflicts before final review
- after merge, pull `main` locally and rerun quick checks
- if artifacts changed, verify teammates know whether to regenerate or pull updated files

### 15.6 Suggested Release Process

There is no formal versioning pipeline yet; this is a practical baseline:

1. Cut a release branch from `main`.
2. Run full validation (backend compile, frontend build, smoke API call, any required retraining).
3. Tag the release commit (`vX.Y.Z`) after merge.
4. Publish release notes summarizing user-facing changes, data/model updates, and migration steps.
